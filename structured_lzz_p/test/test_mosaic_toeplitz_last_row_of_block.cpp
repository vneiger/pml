...
void check(long p)
{
    ...
    for (long i = 2; i < 10; i += 1)
    {
        ...
        Vec<zz_p> row;
        row = MH.last_row_of_block(0);

        for (long j = 0; j < i; j++)
            assert (row[j] == h00(1, j));
        for (long j = 0; j < 2; j++)
            assert (row[j+i] == h01(1, j));
        for (long j = 0; j < i; j++)
            assert (row[j+i+2] == h02(1, j));
        
        row = MH.last_row_of_block(1);
        for (long j = 0; j < i; j++)
            assert (row[j] == h10(i-2, j));
        for (long j = 0; j < 2; j++)
            assert (row[j+i] == h11(i-2, j));
        for (long j = 0; j < i; j++)
            assert (row[j+i+2] == h12(i-2, j));
    }
}
