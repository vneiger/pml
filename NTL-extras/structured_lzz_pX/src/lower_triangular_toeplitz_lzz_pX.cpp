...
/*------------------------------------------------------------*/
void lower_triangular_toeplitz_lzz_pX::mul_right(Mat<zz_pX>& res, const Mat<zz_pX>& input) const
{
    if (input.NumRows() != n)
    {
        LogicError("Bad size for lower_triangular_toeplitz_lzz_pX right matrix multiplication.");
    }
