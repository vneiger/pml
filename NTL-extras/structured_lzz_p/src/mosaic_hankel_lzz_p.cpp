...
/*------------------------------------------------------------*/
void mosaic_hankel_lzz_p::mul_right(Mat<zz_p>& res, const Mat<zz_p>& input) const
{
    if (input.NumRows() != m)
    {
...
/*------------------------------------------------------------*/
void mosaic_hankel_lzz_p::mul_left(Mat<zz_p>& res, const Mat<zz_p>& input) const
{
    if (input.NumCols() != n)
    {
...
/*------------------------------------------------------------*/
void mosaic_hankel_lzz_p::phi_plus_generators(Mat<zz_p>& G, Mat<zz_p>& H) const
{
    ...
    {
        first_column_of_block(tmp_col, i);
        G[0][i] = tmp_col[n-1];
        for (long j = 1; j < n; j++)
            G[j][i] = tmp_col[j-1];
        ...
        if (i > 0)
        {
            last_column_of_block(tmp_col, i-1);
            for (long j = 0; j < n; j++)
...
/*------------------------------------------------------------*/
long mosaic_hankel_lzz_p::solve(Vec<zz_p>& x, const Vec<zz_p> b, long thresh, long thresh_alpha) const
{
    ...
    return 1;
//    CL = De X M Y^t Df
//    M x = b -> De X M x = De X b -> De X M Y^t Df Df^-1 Y^-t x = De X b 
//    b' = De X b -> CL Df^-1 Y^-t x = b' -> CL x' = b' and x = Y^t Df x'
//    (for transpose solve)
//    CL^t = Df Y M^t X^t Dd
//    M^t x = b -> Df Y M^t x = Df Y b -> Df Y M^t X^t De De^-1 X^-t x = Df Y b 
//    b' = Df Y b -> CL^t De^-1 X^-t x = b' -> CL^t x' = b' and x = X^t De x'
}
...
/* thresh_alpha switches between block and plain quadratic    */
/*------------------------------------------------------------*/
long mosaic_hankel_lzz_p::inv(hankel_like_minus_lzz_p& inv, long thresh, long thresh_alpha) const
{
    ...
    inv = hankel_like_minus_lzz_p(iG, iH);
    return 1;
}
