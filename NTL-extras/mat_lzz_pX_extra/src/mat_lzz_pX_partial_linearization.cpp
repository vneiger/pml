                                                    ...
                                                    )
{
    ...
    for (long j = 1; j < pmat.NumCols()+1; ++j)
    {
        double dmax = std::min<long>(target_degree[j-1], dsup) - dinf + 1.0;
        if (dmax < 0.0)
            dmax = 0.0;
        parlin_cols[j] = parlin_cols[j-1] + ceil(dmax / (parlin_degree[j-1]+1));
    }
                          ...
                          )
{
    ...
    Mat<zz_pX> b_parlin;
    std::vector<long> parlin_cols = column_partial_linearization(b_parlin,b,parlin_degree,target_degree);
    ...
    for (long i=0; i<a.NumRows(); ++i)
    {
        ...
        {
            c[i][j].SetLength(deg_a + target_degree[j] + 1);
            long d_init=0;
            for (long jj=parlin_cols[j]; jj<parlin_cols[j+1]; ++jj)
            {
