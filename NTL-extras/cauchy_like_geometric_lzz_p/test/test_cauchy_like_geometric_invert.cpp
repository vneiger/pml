...
void test(long i, long j, long alpha, long rk)
{
    ...
    cauchy_like_geometric_lzz_p M, invM;
    ...
    r = invert_leading_principal_minor(invM, M, 50, 8);
    invZ = invM.to_dense();
    assert (r == rk);
    Z2 = Z;
    assert (r == gauss(Z2));
    ...
    r = invert_leading_principal_minor(invM, M, 10, 15);
    assert (r == rk);
    Z2 = Z;
    assert (r == gauss(Z2));
    subZ.SetDims(r, r);
    ...
    r = invert_leading_principal_minor(invM, M, 10000, 15);
    assert (r == rk);
    Z2 = Z;
    assert (r == gauss(Z2));
    subZ.SetDims(r, r);
    ...
    if (rk < min(i, j)-1) 
    {
        ...
        M = cauchy_like_geometric_lzz_p(A, B, to_zz_p(1), power(a, i), a);
        r = invert_leading_principal_minor(invM, M, 5, 10);
        assert (r == -1);

        r = invert_leading_principal_minor(invM, M, 100, 10);
        assert (r == -1);

        r = invert_leading_principal_minor(invM, M, 10000, 10);
        assert (r == -1);
    }
}
