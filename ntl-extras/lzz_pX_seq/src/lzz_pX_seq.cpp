...
void kurakin(const long d, const Vec<zz_pX> &S, Vec<zz_pXY> &gens){
  ...
  std::map<long, Vec<zz_pX>> I_seq;
  std::map<long, zz_pXY> I_fs;
  ...
  for (long t = 0; t < d; t++){
    ...
    if (verbose){
      ...
      std::cout << "f_t: " << gens[t] << std::endl;
      std::cout << "u_s[t]" << std::endl;
      for (long i = 0; i < us[t].length(); i++)
	std::cout << us[t][i] << std::endl;
      std::cout << std::endl;
    }
  ...
  for (long s = 1; s < S.length(); s++){
	...
	if (it != I_seq.end() && k >= 0){
	  ...
	  if (valuation(l) < valuation(r))
	    ...
	    if(verbose){
	      std::cout << "f after: " << f << std::endl << std::endl;
	    }
	  }
	}else{
...
void modified_kurakin(const long d, const Vec<zz_pX> &S, Vec<zz_pXY> &gens){
  gens.SetLength(0);
  std::map<long, Vec<zz_pX>> us; // keeps track of the current sequences
  std::map<long, zz_pXY> fs; // keeps track of the current poly
  ...
  std::vector<bool> bitmap; // bitmap[i] = true if x^t is potentially useful
  ...
  for (long s = 1; s < S.length(); s++){
    ...
    while (!Q.empty()){
      ...
      if(verbose){
	std::cout << "start at (s,t): (" << s << ", " << t << ")" << std::endl;
	std::cout << "u: " << u << std::endl;
	std::cout << "f: " << f << std::endl;
      }
...
typedef std::pair<Vec<Module>, zz_pXY> mypair;
...
Vec<zz_pX> solve(const Vec<zz_pX> &l, const Vec<Vec<zz_pX>> &rs, const long d){
  ...
  pmbasis(appbas,F,d,shifts);
  if (verbose){
    std::cout << "appbas: " << appbas << std::endl;
  }
  ...
  for (long r = 0; r < n+1; r++){
    if (verbose){
      std::cout << "(r,n+1): " << r << " " << n << std::endl;
      std::cout << "looking at: " << appbas[r][n] << std::endl;
    }
...
Vec<zz_pX> solve(const Vec<zz_pX> &l, const Vec<Vec<zz_pX>> &rs, const long d,
  ...
  if (ind == -1){
    ...
    if (verbose) std::cout << "minval: " << min_val << std::endl;
    tprime = min_val;
    if (verbose) std::cout << "tprime: " << tprime << std::endl;
    return soln;
  }
  zz_pX invC = InvTrunc(appbas[ind][n], d);
  soln.SetLength(n);
...
void kurakin(const long d, const Vec<Module> &S, Vec<zz_pXY> &gens){ 
  std::map<long, std::vector<mypair>> I;
  Vec<Vec<Module>> us;
  us.SetLength(d);
  gens.SetLength(d);
  ...
  for (long t = 0; t < d; t++){
    ...
    auto it = I.find(k);
    if (it == I.end() && k >= 0){
      // create a std::pair and add to I
  ...
  for (long s = 1; s < S.length(); s++){
    ...
    for (long t = 0; t < d; t++){
      ...
      if (verbose){
	std::cout << std::endl << std::endl <<  "AT: " << s << " " << t << std::endl;
	std::cout << "f: " << f << std::endl;
	std::cout << "u: " << u << std::endl;
      }
	...
	if (it != I.end()){
	  ...
	  if (ind == -1){ // no dependency, just add to basis
	    ...
	    // inside I[k]
	    if (verbose){
	      std::cout << "vector size: " << I[k].size() << std::endl;
...
void modified_kurakin(const long d, const Vec<Module> &S, Vec<zz_pXY> &gens){
  gens.SetLength(0);
  std::map<long, std::vector<mypair>> I;
  std::map<long, Vec<Module>> us; // current sequences;
  std::map<long, zz_pXY> fs; // current polys

  std::vector<bool> bitmap; // bitmap[i] = true if x^t is potentially useful
  bitmap.resize(d,false);

  if (verbose) std::cout << "\nStarting mo. kurakin" << std::endl;
...
void berlekamp_massey_pmbasis(const long d, const Vec<Module> &S,
  ...
  if (rank_check){
    ...
    long K = kern.NumRows();
    std::cout << "rank of F: " << e+1-K << std::endl;
    ...
    long rank = gauss(Hc);
    std::cout << "rank of F(0): " << rank << std::endl;
  }
}
...
void right_mul(Vec<zz_p> &out, const Vec<zz_p> seq, const Vec<zz_p> v){
  ...
  auto C = A*B;
  std::cout << "time single mul: " << GetWallTime()-t << std::endl;
  out.SetLength(e+1);
  for (long i = 0; i < e+1;i++)
    out[i] = coeff(C,i+(e-1));
}
...
*/
 /*
  ...
  prod = H*proj;
  std::cout << "time to verify: " << GetWallTime()-time_ver << std::endl;
*/
}
