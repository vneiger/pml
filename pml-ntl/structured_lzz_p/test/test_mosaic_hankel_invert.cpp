    ...
    for (long rep = 0; rep < 100; rep++)
        for (long i = 1; i < 5; i++)
            for (long j = 1; j < 5; j++)
            {
                ...
                Vec<Vec<hankel_lzz_p>> mat;
                mat.SetLength(i);
                for (long k = 0; k < i; k++)
                {
                    Vec<hankel_lzz_p> row;
                    row.SetLength(j);
                    for (long ell = 0; ell < j; ell++)
                        row[ell] = hankel_lzz_p(random_vec_zz_p(rows[k] + cols[ell] - 1), rows[k], cols[ell]);
                    mat[k] = row;
                }
                mosaic_hankel_lzz_p MH = mosaic_hankel_lzz_p(mat);
                long n = MH.NumCols();

                assert (n == MH.NumRows());
                hankel_like_minus_lzz_p inv;
                long r = MH.inv(inv);
