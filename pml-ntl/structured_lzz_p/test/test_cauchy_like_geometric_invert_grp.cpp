...
void test(long i, long j, long alpha, long rk)
{
    ...
    cauchy_like_geometric_lzz_p M, Mbak, invM;
    Mat<zz_p> A, B, Z, invZ, Z2, subZ;
    long r, s;
    zz_p a;
    ...
    // 1. generic rank profile
    {
        ...
        // also checks that output can alias input
        M = Mbak;
        r = M.invert_leading_principal_minor_grp(M, 50, 8);
        assert (r == 1);
        invZ = M.to_dense();
        s = invZ.NumRows();
        assert (s == rk);
        ...
        for (long u = 0; u < s; u++)
            for (long v = 0; v < s; v++)
                subZ[u][v] = Z[u][v];
        subZ = subZ*invZ;
        assert (IsIdent(subZ, s));
        
        M = Mbak;
        r = M.invert_leading_principal_minor_grp(M, 10, 15);
        assert (r == 1);
        invZ = M.to_dense();
        s = invZ.NumRows();
        assert (s == rk);
        ...
        M = Mbak;
        r = M.invert_leading_principal_minor_grp(M, 10000, 15);
        assert (r == 1);
        invZ = M.to_dense();
        s = invZ.NumRows();
        assert (s == rk);
    ...
    if (rk < min(i, j)-1) 
    {
        ...
        M = Mbak;
        r = M.invert_leading_principal_minor_grp(M, 5, 10);
        assert (r == 0);

        M = Mbak;
        r = M.invert_leading_principal_minor_grp(M, 100, 10);
        assert (r == 0);

        M = Mbak;
        r = M.invert_leading_principal_minor_grp(M, 10000, 10);
        assert (r == 0);
    }
}
