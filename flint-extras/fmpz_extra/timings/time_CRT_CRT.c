...
/*--------------------------------------------------------------*/
/*--------------------------------------------------------------*/
void check_fmpz_CRT_CRT(ulong num_primes)
{
    flint_rand_t state;
    fmpz_t comb;
    ...
    fmpz_multimod_init(mmod, primes, num_primes, num_primes*(FLINT_BITS-8));
    ...
    {
        tt = clock();
        fmpz_CRT_CRT(comb, residues, mCRT);
    ...
    {
        tt = clock();
        fmpz_multimod_reduce(residues, comb, mmod);
        t += (double)(clock()-tt) / CLOCKS_PER_SEC;
        ++nb_iter;
    }
    t = 1000 * t;
