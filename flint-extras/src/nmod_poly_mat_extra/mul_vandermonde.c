...
void nmod_poly_mat_mul_vandermonde1(nmod_poly_mat_t c, const nmod_poly_mat_t a, const nmod_poly_mat_t b)
{
    ...
    if (c == a || c == b)
    {
        ...
        nmod_poly_mat_mul_vandermonde1(T, a, b);
    ...
    for (i = 0; i < m; i++)
        for (j = 0; j < n; j++, ell++)
        {
            ulong d;
            nn_ptr ptr;
            d = nmod_poly_mat_entry(a, i, j)->length;
            ptr = nmod_poly_mat_entry(a, i, j)->coeffs;
            for (k = 0; k < d; k++)
                nmod_mat_entry(tmp_mat, k, ell) = ptr[k];
        }
    // note: all non-touched entries already zero since tmp_mat was initialized as zero
    ...
    for (i = 0; i < n; i++)
        for ( j = 0; j < p; j++, ell++)
        {
            ulong d;
            nn_ptr ptr;
            d = nmod_poly_mat_entry(b, i, j)->length;
    ...
    for (u = 0; u < m; u++)
        ...
        {
            nn_ptr coeffs;
            nmod_poly_fit_length(nmod_poly_mat_entry(c, u, v), nb_points);
            _nmod_poly_set_length(nmod_poly_mat_entry(c, u, v), nb_points);
...
void nmod_poly_mat_mul_vandermonde2(nmod_poly_mat_t c, const nmod_poly_mat_t a, const nmod_poly_mat_t b)
{
    ...
    if (c == a || c == b)
    {
        nmod_poly_mat_t T;
        nmod_poly_mat_init(T, m, n, p);
        nmod_poly_mat_mul_vandermonde2(T, a, b);
        nmod_poly_mat_swap_entrywise(c, T);
        nmod_poly_mat_clear(T);
        return;
    }
    ...
    for (i = 0; i < m; i++)
        for (j = 0; j < n; j++, ell++)
        {
            ...
            if (d % 2 == 1)
                nmod_mat_entry(tmp_mat_even, d/2, ell) = ptr[d-1];
        }
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < p; j++, ell++)
        {
            ...
            if (2*k+1 == d)
            {
                nmod_mat_entry(tmp_mat_even, k, ell) = ptr[d-1];
