...
/* test one given input for kernel_zls (for the moment, approx only) */
int core_test_left_kernel_zls(nmod_poly_mat_t mat, slong * shift)
{
    int res = 1;
    int local_res = 1;
...
int collection_test_kernel(slong iter, flint_rand_t state)
{
    /* input matrix and shift */
    nmod_poly_mat_t mat;
    slong * shift;
            ...
            for (slong i_cdims = 0; i_cdims < _test_collection_nb_dims; i_cdims++)
                for (slong i_degs = 0; i_degs < _test_collection_nb_smalldegs; i_degs++)
                    for (slong it = 0; it < iter; it++)
                    {
                        ...
                        if (! core_test_left_kernel_zls(mat, shift))
                        ...
                        _test_collection_shift_uniform(shift, rdim);
                        _test_collection_mat_unbalanced_rdeg(mat, len, state);
                        if (! core_test_left_kernel_zls(mat, shift))
                        { flint_printf("failed %s -- %s,\n...exiting\n", "uniform", "unbalanced rdeg"); return 0; }

                        _test_collection_shift_uniform(shift, rdim);
                        _test_collection_mat_unbalanced_cdeg(mat, len, state);
                        if (! core_test_left_kernel_zls(mat, shift))
                        ...
                        /* HNF shift */
                        /* matrix: zero | uniform | rdeg | cdeg | test | sparse | rkdef */
                        _test_collection_shift_hermite(shift, rdim, len);
                        _test_collection_mat_zero(mat);
                        if (! core_test_left_kernel_zls(mat, shift))
                        ...
                        /* HNF shift, bis */
                        /* matrix: zero | uniform | rdeg | cdeg | test | sparse | rkdef */
                        _test_collection_shift_rhermite(shift, rdim, len);
                        _test_collection_mat_zero(mat);
                        if (! core_test_left_kernel_zls(mat, shift))
                        ...
                        /* plateau shift */
                        /* matrix: zero | uniform | rdeg | cdeg | test | sparse | rkdef */
                        _test_collection_shift_plateau(shift, rdim, len);
                        _test_collection_mat_zero(mat);
                        if (! core_test_left_kernel_zls(mat, shift))
                        ...
                        /* plateau shift, bis */
                        /* matrix: zero | uniform | rdeg | cdeg | test | sparse | rkdef */
                        _test_collection_shift_rplateau(shift, rdim, len);
                        _test_collection_mat_zero(mat);
                        if (! core_test_left_kernel_zls(mat, shift))
...
TEST_FUNCTION_START(nmod_poly_mat_kernel, state)
{
    ...
    if (0)
    {
        ...
        {
            {
                result = collection_test_kernel(1, state);
