...
static ulong _random_modulus(flint_rand_t state)
{
...
TEST_FUNCTION_START(nmod_poly_mat_mul_sd_fft_direct, state)
{
    slong i;

    for (i = 0; i < 300 * flint_test_multiplier(); i++)
    {
        const ulong modn = _random_modulus(state);
        ...
        nmod_poly_mat_mul_sd_fft_direct(C2, A, B);
        ...
        if (m == k && k == n)
        {
            ...
            nmod_poly_mat_mul_sd_fft_direct(C2, C2, B);
            if (!nmod_poly_mat_equal(C1, C2))
                TEST_FUNCTION_FAIL("aliasing C == A: modn = %wu, m = %wd, k = %wd, n = %wd\n",
                                   modn, m, k, n);
            nmod_poly_mat_set(C2, B);
            nmod_poly_mat_mul_sd_fft_direct(C2, A, C2);
            if (!nmod_poly_mat_equal(C1, C2))
