...
void nmod_poly_mat_row_degree_zero(slong *rdeg, const nmod_poly_mat_t mat, const slong *shift)
{
    if (shift == NULL)
    {
        slong max, d;
    ...
    else
    {
        ...
        for (slong i = 0; i < mat->r; i++)
        {
            max = min_shift; // zero rows will have this as rdeg
            for (slong j = 0; j < mat->c; j++)
            {
                d = nmod_poly_degree(nmod_poly_mat_entry(mat, i, j)) + shift[j];
                // if new maximum at a nonzero entry, update
...
void nmod_poly_mat_column_degree_zero(slong *cdeg, const nmod_poly_mat_t mat, const slong *shift)
{
    if (shift == NULL)
    {
        slong max, d;
