...
/* for charpoly/minpoly of dim x dim */
typedef struct {
    slong dim;
    slong modn;
} time_args;

#define TIME_CPMP(fun) \
...
TIME_CPMP(nmod_mat_charpoly)
TIME_CPMP(nmod_mat_charpoly_danilevsky)
TIME_CPMP(nmod_mat_charpoly_berkowitz)
TIME_CPMP(nmod_mat_minpoly)

SAMPLE_CPMP(nmod_mat_charpoly)
SAMPLE_CPMP(nmod_mat_charpoly_danilevsky)
SAMPLE_CPMP(nmod_mat_charpoly_berkowitz)
SAMPLE_CPMP(nmod_mat_minpoly)
...
int main(int argc, char ** argv)
{
    ...
    const timefun funs[] = {
        time_nmod_mat_minpoly,              // 0
        time_nmod_mat_charpoly,             // 1
        time_nmod_mat_charpoly_danilevsky,  // 2
        time_nmod_mat_charpoly_berkowitz,   // 3
    };

    typedef void (*samplefun) (void*, ulong);
    const samplefun sfuns[] = {
        sample_nmod_mat_minpoly,              // 0
        sample_nmod_mat_charpoly,             // 1
        sample_nmod_mat_charpoly_danilevsky,  // 2
        sample_nmod_mat_charpoly_berkowitz,   // 3
    };

    const char * description[] = {
        "#0  --> nmod_mat_minpoly              ",
        "#1  --> nmod_mat_charpoly             ",
        "#2  --> nmod_mat_charpoly_danilevsky  ",
        "#3  --> nmod_mat_charpoly_berkowitz   ",
    };
