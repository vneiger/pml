...
/*------------------------------------------------------------*/
void nmod_double_fft_init_set(nmod_double_fft_t F, mp_limb_t w, ulong order, nmod_t mod)
{
    vec1d inv_2, inv;
    ulong k;
    double dp;    
    ...
    F->powers_inv_w_t = (vec1d **) flint_malloc(sizeof(vec1d *) * (order + 1));
    ...
    for (k = 0; k <= order; k++)
    {
        ...
        if ((sizeof(vec1d) * K) >= 32)
        {
            ...
            F->powers_inv_w_t[k] = (vec1d *) aligned_alloc(32, sizeof(vec1d) * K);
        }
        else
        {
            F->powers_w[k] = (vec1d *) flint_malloc(sizeof(vec1d) * K);
            F->powers_inv_w_t[k] = (vec1d *) flint_malloc(sizeof(vec1d) * K);
        }
        ...
        else
        {
            ...
            while (K >= 1)
            {
                ...
                for (i = 0; i < K; i++)
                {
                    F->powers_w[k][nb] = (vec1d) wri - dp;
                    wri = nmod_mul(wri, wr, mod);
                    F->powers_inv_w_t[k][nb] = (vec1d) inv_wri - dp;
    ...
    F->powers_inv_2 = (vec1d *) flint_malloc(sizeof(vec1d) * (order + 1));
    inv_2 = nmod_inv(2, mod);
    inv = 1;
    for (k = 0; k <= order; k++)
    {
        F->powers_inv_2[k] = (vec1d) inv - dp;
    ...
    for (k = 0; k < order; k++)
    {
        ...
        for (i = 0; i < K; i++)
        {
            F->powers_inv_w_over_2[k][i] = (vec1d) nmod_mul(src[i], F->powers_inv_2[k], mod) - dp;
        }
    }
}
