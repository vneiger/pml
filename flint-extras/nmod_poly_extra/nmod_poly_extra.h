...
/*------------------------------------------------------------*/
/*------------------------------------------------------------*/
/* experimental FFT using doubles                             */
/*------------------------------------------------------------*/
/*------------------------------------------------------------*/
...
typedef struct
{
    ...
    /*  total length K-1 (we allocate K)                                            */ 
    double **powers_w;
    double **powers_inv_w_t;  // same table for 1/w as for w

    // length order+1, with powers_inv_2[i] = 1/2^i 
    double *powers_inv_2;
    
    // length order, level k has length 2^k with entries 1/w{k+1}^i/2^k 
    double **powers_inv_w_over_2;
} nmod_double_fft_struct;
typedef nmod_double_fft_struct nmod_double_fft_t[1];
...
/* clears all memory assigned to F                            */
/*------------------------------------------------------------*/
void nmod_double_fft_clear(nmod_double_fft_t F);
