...
uint _nmod32_vec_dot_split_avx2_acc8(n32_srcptr vec1, n32_srcptr vec2, slong len, nmod_t mod, ulong pow2_precomp)
{
    const __m256i low_bits = _mm256_set1_epi64x(DOT_SPLIT_MASK);
    __m256i dp_lo0 = _mm256_setzero_si256();
    __m256i dp_hi = _mm256_setzero_si256();
...
uint _nmod32_vec_dot_split_avx512(n32_srcptr vec1, n32_srcptr vec2, slong len, nmod_t mod, ulong pow2_precomp)
{
    ...
    __m512i dp_hi = _mm512_setzero_si512();
    ...
    for ( ; i+31 < len; i += 32)
    {
        ...
        dp_hi = _mm512_add_epi64(dp_hi, _mm512_srli_epi64(dp_lo0, DOT_SPLIT_BITS));
    ...
    ulong hsum_hi = _mm512_hsum(dp_hi) + (hsum_lo >> DOT_SPLIT_BITS);
    hsum_lo &= DOT_SPLIT_MASK;
...
uint _nmod32_vec_dot_split_avx512_acc8(n32_srcptr vec1, n32_srcptr vec2, slong len, nmod_t mod, ulong pow2_precomp)
{
    const __m512i low_bits = _mm512_set1_epi64(DOT_SPLIT_MASK);
    __m512i dp_lo0 = _mm512_setzero_si512();
    __m512i dp_hi = _mm512_setzero_si512();
