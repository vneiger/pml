...
ulong
_nmod_vec_newdot(nn_srcptr vec1, nn_srcptr vec2, slong len, nmod_t mod, dot_params_t params)
{
...
ulong
_nmod_vec_newdot_rev(nn_srcptr vec1, nn_srcptr vec2, slong len, nmod_t mod, dot_params_t params)
{
    ulong res = UWORD(0);   /* covers _DOT0 */
    slong i;
...
#ifdef HAVE_AVX2
    {
        ...
        _NMOD_VEC_DOT2_32(res, i, len, vec1[i], vec2[len-1-i], mod)
    else if (params.method == _DOT2)
...
void nmod_mat_mul_newdot1(nmod_mat_t C, const nmod_mat_t A, const nmod_mat_t B)
{
    ...
    for (slong i = 0; i < A->r; i++)
        for (slong j = 0; j < B->r; j++)
            C->rows[i][j] = _nmod_vec_newdot(A->rows[i], B->rows[j], A->c, A->mod, params);
}
