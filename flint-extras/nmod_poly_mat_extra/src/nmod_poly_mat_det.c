...
void nmod_poly_mat_det_iter_bis(nmod_poly_t det, nmod_poly_mat_t mat)
{
    ...
    view->modulus = mat->modulus; // TODO remove once flint bug fixed
    ...
    for (slong i = mat->r -1; i >= 1; i--)
    {
        ...
        // with early exit if detecting rank < i (in which case rk < 0)
        // with update of the determinant of unimodular transformation (+1 or -1)
        rk = _nmod_poly_mat_weak_popov_lr_iter_submat_rowbyrow(view, NULL, NULL, &udet, pivind, NULL, 0, 0, i+1, i, 2);
